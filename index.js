    ...
    function replaceValues(obj) {
        for (const key in obj) {
            ...
            if (val !== null && typeof val === 'object') {
                ...
                if (val === "*ninput*") obj[key] = s.customNegative || "";
                if (val === "*seed*") { obj[key] = finalSeed; seedInjected = true; }
                if (val === "*sampler*") obj[key] = s.selectedSampler || "euler";
                if (val === "*model*") obj[key] = s.selectedModel || "v1-5-pruned.ckpt";

                if (val === "*steps*") obj[key] = parseInt(s.steps) || 20;
                if (val === "*cfg*") obj[key] = parseFloat(s.cfg) || 7.0;
...
async function onImageSwiped(data) {
    ...
    // Allow overswipe on native Image Gen Kazuma messages, OR any message where the currently viewed image was generated by us
    const media = message.extra?.media || [];
    const idx = message.extra?.media_index || 0;
    
    if (message.name !== "Image Gen Kazuma") {
        const currentMedia = media[idx];
        if (!currentMedia || currentMedia.source !== "generated") return;
    }
...
jQuery(async () => {
    try {
        ...
        // SillyTavern natively blocks swiping right if there's only 1 image, showing a toast.
        // We intercept touch events on images to detect left-swipes (next image) manually.
        let chatTouchStartX = 0;
        $(document).on('touchstart', '.mes_media_container img, .inline-image-container img', function(e) {
            if (e.originalEvent && e.originalEvent.touches) {
                chatTouchStartX = e.originalEvent.touches[0].screenX;
            }
        });
        $(document).on('touchend', '.mes_media_container img, .inline-image-container img', function(e) {
            if (e.originalEvent && e.originalEvent.changedTouches) {
                const touchEndX = e.originalEvent.changedTouches[0].screenX;
                const threshold = 40;
                if (touchEndX < chatTouchStartX - threshold) { // Swiped Left (Next)
                    ...
                    if (mesId !== undefined) {
                        ...
                        // If it's a 1-image gallery, or we are on the very last image
                        if (media.length <= 1 || idx >= media.length - 1) {
                            // Only allow if it's an Image Gen message, or the current image was generated by us
                            let allow = false;
                            if (message.name === "Image Gen Kazuma") allow = true;
                            else if (media[idx] && media[idx].source === "generated") allow = true;
                            
                            if (allow) {
                                e.preventDefault();
                                e.stopPropagation();
                                e.stopImmediatePropagation();
                                onImageSwiped({ message: message, direction: "right", element: this });
                            }
                        }
                    }
                }
        ...
        window._kazumaTouchEnd = function(e) {
            ...
            if (e.changedTouches) {
                ...
                if (touchEndX < touchStartX - threshold) { // Swipe Left (Next)
                    const $next = $swipeContainer.find('.right_menu_button, .right_arrow, i.fa-chevron-right, .fa-chevron-right, .fa-arrow-right, [title*="Next"], [title*="next"]').closest('div, button, a, span');
                    if ($next.length && $next.is(':visible') && !$next.hasClass('disabled')) { 
                        e.preventDefault(); e.stopPropagation();
                        $next.click(); 
                        updateModalImg(); 
                    } else {
                        // OVERSWIPE: No next arrow, or it's disabled. Trigger generation manually!
                        const $mes = sourceImage.closest('.mes');
                        const mesId = $mes.attr('mesid');
                        if (mesId !== undefined) {
                            const message = getContext().chat[mesId];
                            if (message) {
                                e.preventDefault(); e.stopPropagation();
                                onImageSwiped({ message: message, direction: "right", element: sourceImage[0] });
                            }
                        }
                    }
                }
                else if (touchEndX > touchStartX + threshold) { // Swipe Right (Prev)
                    const $prev = $swipeContainer.find('.left_menu_button, .left_arrow, i.fa-chevron-left, .fa-chevron-left, .fa-arrow-left, [title*="Prev"], [title*="prev"]').closest('div, button, a, span');
                    if ($prev.length) { 
                        e.preventDefault(); e.stopPropagation();
                        $prev.click(); 
                        updateModalImg(); 
